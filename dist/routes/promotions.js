...
router.put('/:id', [
    ...
    (0, express_validator_1.check)('id', 'Id should be a number').isNumeric(),
    (0, express_validator_1.check)('rules.*.ruleType', 'Rule type should be a string').isString(),
    (0, express_validator_1.check)('rules.*.ruleType', `ruleType type should be: ${rules_1.rulesType.join(' or ')}`).isIn(rules_1.rulesType),
    (0, express_validator_1.check)('rules.*.actions', 'Rule type should be a array').isArray(),
    (0, express_validator_1.check)('rules.*.actions.*.actionType', `actionType type should be: ${actions_1.actionsType.join(' or ')}`).isIn(actions_1.actionsType),
    (0, express_validator_1.check)('rules.*.actions.*.discountType', `discountType type should be: ${actions_1.discountsType.join(' or ')}`).isIn(actions_1.discountsType),
    validatePeriod_1.validatePeriod,
    validateActivatedPromotions_1.validateActivatedPromotions,
    validateFields_1.validateFields,
